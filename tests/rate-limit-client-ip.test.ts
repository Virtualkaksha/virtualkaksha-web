...
test("missing or invalid trusted-proxy configuration fails explicitly", () => {
  assert.deepEqual(
    ...
    { ok: false, code: "INVALID_CONFIGURATION", message: "RATE_LIMIT_TRUSTED_PROXY must select a valid mode for the current environment." },
  );
  assert.equal(
...
test("Vercel mode trusts only the first x-forwarded-for address", () => {
  const result = resolveTrustedClientIp({
    ...
    environment: { ...rateLimitEnvironment, RATE_LIMIT_TRUSTED_PROXY: "vercel" },
  });
...
test("direct mode ignores every forwarded header", () => {
  const result = resolveTrustedClientIp({
    ...
    environment: { ...rateLimitEnvironment, RATE_LIMIT_TRUSTED_PROXY: "direct" },
  });
...
test("test injection is deterministic and forbidden in production", () => {
  ...
  assert.deepEqual(resolveTrustedClientIp({
    ...
    environment: { ...rateLimitEnvironment, NODE_ENV: "production", RATE_LIMIT_ADAPTER: "upstash" },
  }), {
    ok: false,
