...
# D3 atomic metadata apply

The ADMIN import page can apply the original CSV only after a fresh server-side preview and the exact confirmation `APPLY <N> RESOURCES`, where N is the meaningful changed-row count. Apply is limited to 25 total rows and commits all changes and audits in one Serializable transaction; no-op rows are untouched. Published metadata changes return to pending review. Stale retries conflict without a second write. Preserve an unchanged copy of the latest template before editing. There is no durable batch history or idempotency record yet; D4 will address that limitation.
