...
export default async function TeacherResourcesPage({ searchParams }: Props) {
  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-sm font-semibold text-blue-700">Teacher CMS</p><h1 className="mt-1 text-3xl font-bold text-slate-950">My Resources</h1><p className="mt-2 text-sm text-slate-600">Search and manage resources you created.</p></div>
      <div className="flex items-center gap-3"><div className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm text-slate-600"><strong className="text-slate-950">{total}</strong> matching resources</div><Link href="/teacher/resources/new" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-bold text-white"><PlusCircle size={17}/> Add resource</Link></div>
    </header>
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-sm">
        <thead className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Resource</th><th className="px-3 py-3">Academic path</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Access</th><th className="px-3 py-3">PDF asset</th><th className="px-3 py-3">Dates</th><th className="px-3 py-3 text-right">Actions</th></tr></thead>
        <tbody className="divide-y divide-slate-100">{cms.search.items.map((resource) => <tr key={resource.id}>
          <td className="px-3 py-4"><p className="font-semibold text-slate-950">{resource.title}</p><p className="mt-1 text-xs text-slate-500">{resource.resourceType.name} · {resource.format.replaceAll("_", " ")}</p></td>
          <td className="px-3 py-4 text-slate-600">{resource.academicLabel}</td>
