...
export default async function AdminResourcesPage({ searchParams }: { searchParams: Promise<{ status?: string; q?: string; archived?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-7 px-4 py-8 sm:px-6 lg:px-8">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-blue-700">Resource moderation</p><h1 className="mt-1 text-3xl font-bold text-slate-950">Review queue</h1><p className="mt-2 text-sm text-slate-600">Preview teacher submissions and approve, reject or archive them.</p></div><Link href="/admin/resources/inventory" className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800">Resource inventory</Link></header>
