...
export default async function PublicSearchPage({ searchParams }: { searchParams: Promise<RawSearchParams> }) {
  ...
  const description = query.type && presentation.description
    ...
    : "Browse and open published free resources without signing in. Create an account to bookmark material and save reading progress.";
  return <><Navbar /><main className="mx-auto w-full max-w-7xl flex-1 px-6 py-12"><header><h1 className="text-4xl font-bold text-slate-950">{presentation.heading}</h1><p className="mt-3 text-slate-600">{description}</p></header>
