...
export async function main(arguments_ = process.argv.slice(2), environment = process.env): Promise<void> {
  ...
  try {
    ...
    const result = await createAccountSecurityRepository(prisma).changeRole(user.id, "ADMIN", "add");
    if (result.outcome === "not-found") throw new Error("The requested account or role is unavailable.");
    console.log(result.outcome === "changed" ? "Administrator role assigned; existing sessions were revoked." : "Administrator role was already assigned; no change was made.");
  } finally {
...
if (import.meta.url === invokedPath) {
  main().catch(() => {
    console.error("Administrator role assignment failed.");
    process.exitCode = 1;
  });
}
